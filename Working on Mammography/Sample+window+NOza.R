...
ctrl4<-MOAoptions(model="OnlineAccuracyUpdatedEnsemble")
mymodel4<-OnlineAccuracyUpdatedEnsemble(control=ctrl4)

naive_model <- trainMOA(model = mymodel4,a ~ ., data = trainset)
naive_model


emodel1<-OzaBoost(baseLearner="bayes.NaiveBayes",ensembleSize=30)
