...
points2 <- get_points(train_strm,5592)
...
update(clustream1,train_strm,5592)


#Storing the clusters assigned to the input data points to a
a<- get_assignment(clustream1, points2)
a <- as.data.frame(a)
...
reset_stream(train_strm)
write_stream(train_strm, file="InputforClassifier.txt", n=5592,class=FALSE, append = FALSE, sep=",", header=TRUE, row.names=FALSE)
...
sea_test_data <- read.csv("TestF.txt", sep= ",",col.names = c("V1","V2","V3","V4","V5","V6","V7"))
sea_test_datastream <- datastream_dataframe(data=sea_test_data)
testset <- sea_test_datastream$get_points(sea_test_datastream, n = 4473, outofpoints = c("stop", "warn", "ignore"))
